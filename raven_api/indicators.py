...
def mean_annual_flow(df: pd.DataFrame) -> pd.Series:
    """
    Calculate mean annual flow for each site.
    """
    return df.groupby(['site', 'water_year'])['value'].mean().groupby('site').mean().rename("mean_annual_flow")
...
def mean_aug_sep_flow(df: pd.DataFrame) -> pd.Series:
    ...
    return aug_sep.groupby(['site', 'water_year'])['value'].mean().groupby('site').mean().rename("mean_aug_sep_flow")
...
def peak_flow_timing(df: pd.DataFrame) -> pd.Series:
    ...
    peak_days = df.groupby(['site', 'water_year'])[['value', 'doy']].apply(lambda x: x.loc[x['value'].idxmax(), 'doy'])
...
def days_below_efn(df: pd.DataFrame, EFN_threshold: float) -> pd.Series:
    ...
    """
    mean_annual = df.groupby(['site', 'water_year'])['value'].mean().groupby('site').mean()
...
def peak_flows(df: pd.DataFrame) -> pd.Series:
    ...
    """
    return df.groupby(['site', 'water_year'])['value'].max().groupby('site').mean().rename("mean_annual_peak")


def calculate_all_indicators(parquet_path: str, EFN_threshold: float = 0.2, break_point: Optional[int] = None) -> pd.DataFrame:
    """
    ...
    try:
        ...
        if break_point:
            df['subperiod'] = np.where(df['water_year'] <= break_point, f'before_{break_point}', f'after_{break_point}')
        else:
...
@app.get("/indicators/")
def get_indicators(
    parquet_path: str = Query(..., description="Path to the input Parquet file"),
    efn_threshold: float = Query(0.2, ge=0, le=1, description="EFN threshold between 0 and 1"),
    break_point: Optional[int] = Query(None, description="Optional water year to split the period")
):
    """
    API endpoint to return flow indicators from a Parquet file.
    ...
    """
    try:
        df = calculate_all_indicators(parquet_path, efn_threshold, break_point)
